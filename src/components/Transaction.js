      ...
      <tbody>
        {
          users.map((user, i) => {
            return <tr key={i}>
              <td className="py-2 border-b-2 border-color2">{user.name}</td>
              <td className={"py-2 border-b-2 border-color2 capitalize text-white"}>
                <span className={`${user.status === "reading" ? "bg-yellow-500" : (user.status === "done" ? "bg-blue-600" : "bg-red-600")} py-1 px-2 rounded-md`}>{user.status}</span>
              </td>
